(function() {
  ...
  function DiffableStateHelper(ds, data, diff, patch, flagSet, dsGet, dsSet) {
    this.ds = ds;
    this.data = data;
    this.diff = diff;
    this.patch = patch;
  ...
  function DiffableState(_data, root, rootProp, flagSet) {       
    ...
    var diff = flagSet ? new proxy.constructor() : {};
    var patch = flagSet ? new proxy.constructor() : {};
    ...
    var dsGet = function (prop, noSides) {
      if (!(prop in diff) && !noSides) {
        diff[prop] = data[prop] instanceof DiffableStateHelper ? data[prop].ds : (typeof data[prop] !== 'undefined' ? data[prop] : {'$undefined': null});
        if (typeof root !== 'undefined')
          root.diff[rootProp] = diff;
      }
      
      var toReturn = patch[prop] || data[prop];
      return toReturn instanceof DiffableStateHelper ? toReturn.ds : isPOJS(toReturn) && '$set' in toReturn ? toReturn.$set : toReturn;
    };
    ...
    var dsSet = function(prop, value) {
      ...
      patch.__defineGetter__(prop, function() {
        return c instanceof DiffableStateHelper ? (c.flagSet ? {'$set': c.ds} : c.ds) : (typeof c !== 'undefined' ? c : {'$undefined': null});
      });
