(function() {
  ...
  function Checkerboard(conn) {
    ...
    function reattempt() {
      if (attempts.length > 0) {
        ...
        for (var i = 0; i < attempts.length; i++) {
          ...
          if (Object.keys(attempts[i].diff) > 0) {
    ...
    this.try = function(callback) {
      ...
      attempts.push(new Attempt(callback, deferred));
  ...
  function Attempt(diff, patch, callback, deferred) {
    this.diff = state;
    this.patch = patch;
    ...
    function merge(){
      ...
      for (var p in State) {
        if (!isPOJS(State[sanitize(p)]().data))
          toReturn[p] = unStringReplace(log.patch[p] || State[sanitize(p)]().data);
        else
          toReturn[p] = unStringReplace(State[sanitize(p)]().merge());
      }
