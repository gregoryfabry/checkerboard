(function() {
  ...
  function Checkerboard(conn) {
    var workingState = DiffableStateFactory({});
    var flagReady = false;
    var uuid;
    var attempts = [];
    var transactionId = 0;
  ...
  function DiffableStateFactory(data, _prop, _root) {
    ...
    function State(key, val) {
      ...
      if (arguments.length === 2) {
        if (typeof val === 'undefined' || val === null) {
          ...
          // my good friend demorgan
          if (isPOJS(data) && key in data && isPOJS(val) && ((data[key] instanceof Array && val instanceof Array) || !(data[key] instanceof Array || val instanceof Array)))
    ...
    function resolve() {
      ...
      for (p in log.patch)
        if (!isPOJS(log.patch[p]) && log.patch[p] !== null) {
          ...
          if (p in State)
            State[sanitize(p)]().propegatePatch.patched = false;
        }
      for (p in State)
        State[sanitize(p)]().resolve();
    ...
    function apply(newData) {
      for (var p in newData) {
        ...
        else if (p in State && isPOJS(State[p]().data) && ((State[p]().data instanceof Array && newData[p] instanceof Array) || !(State[p]().data instanceof Array || newData[p] instanceof Array)))
  ...
  function stringReplace(obj) {
    ...
    return (typeof newObj !== 'undefined' ? newObj : obj);
  }
