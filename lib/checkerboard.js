  ...
  DiffableStateHelper.prototype.merge = function(_patch) {
    ...
    for (var prop in patch) {
      if (patch.hasOwnProperty(prop)) {
        ...
        if (cur instanceof DiffableStateHelper)  
          cur.merge();
          
        this.data[prop] = isPOJS(cur) && !(cur instanceof DiffableStateHelper) ? (new DiffableState(cur, {'diff': this.diff, 'patch': this.patch}, prop)) : cur;
